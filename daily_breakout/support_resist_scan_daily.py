...
class StockAnalysis:
    def __init__(self, data):
        ...
        self.original_dates = self.df.index.strftime('%b %d, %y').tolist()  # Update after filtering
    ...
    def confirm_breakout(self, breakout_candle, continuation_candles=15):
    ...
    def plot_data(self, start=None, end=None, candles_back=None):
        ...
        for zone in self.zones:
            # Ensure we're using indices that correspond to the dates in dfpl
            fig.add_shape(
                type="rect",
                x0=dfpl_dates[0],
                x1=dfpl_dates[-1],
        ...
        for breakout in self.breakouts:
            ...
            if start <= breakout_index < end:
                # Get the date for the breakout index from the original dates
                date_for_breakout = self.original_dates[breakout_index]
                # Find this date in the dfpl_dates list to get the corresponding plot index
                try:
                    plot_index = dfpl_dates.index(date_for_breakout)
                    fig.add_trace(go.Scatter(
                        x=[dfpl_dates[plot_index]],
                        y=[breakout_price],
                        mode="markers",
                        marker=dict(size=10, color="green"),
                        name="breakout"), row=1, col=1)
                except ValueError:
                    # The date was not found in dfpl_dates, which means it's outside the plotted range
                    pass
